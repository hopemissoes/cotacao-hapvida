...
## Answer style — MANDATORY, project override
...
`--budget 1200` is this project's default (upstream default is 2000): a smaller subgraph
keeps the answer tight. Raise it to `--budget 3000` only when 1200 comes back truncated
and the missing part is load-bearing.

Render the result per `.claude/graphify-estilo.md`: 2-5 lines of Portuguese prose plus one
Mermaid diagram (or a table), citing `source_location` as `arquivo:linha`. Do not echo the
subgraph.
...
- Never invent an edge — not even to close a gap in a diagram. If unsure, use AMBIGUOUS
