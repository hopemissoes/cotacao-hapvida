...
Replace `QUESTION` with the **expanded** query string, `MODE` with `bfs` or `dfs`, and `BUDGET` with the token budget (this project's default is `1200`, upstream is `2000`, or whatever `--budget N` specifies). Then answer based on the subgraph output above, using only what the graph contains.

**Render it per `.claude/graphify-estilo.md`** — that file overrides the format here:
2-5 lines of Brazilian-Portuguese prose, then one Mermaid diagram (`flowchart LR` for a
flow or call chain, `mindmap` for an overview, `sequenceDiagram` for time order, max 12
nodes) or a markdown table for an inventory. Cite `source_location` as `arquivo:linha`.
Mark each asserted edge `✓` extracted / `~` inferred / `?` ambiguous. Never paste the raw
subgraph into the chat.
...
Per `.claude/graphify-estilo.md`, the path is drawn, not narrated: a `flowchart LR` with
**only the nodes on the path** (no neighbours), each hop's relation as the edge label, then
at most 3 lines in Portuguese for what the path means. Hops that are inferred rather than
...
Then present it per `.claude/graphify-estilo.md`, which replaces the "3-5 sentence
explanation" this reference used to ask for: **2 lines** saying what the node is and why it
matters, followed by a connections table (`relação | nó | arquivo:linha | ✓/~/?`). If the
node sits on a flow rather than in a list, use a `flowchart LR` instead of the table.
Portuguese, no preamble, no recap.
